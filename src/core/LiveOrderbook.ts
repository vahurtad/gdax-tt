...
export class LiveOrderbook extends Duplex implements Orderbook {
    ...
    private handleTradeExecutedMessage(msg: TradeExecutedMessage) {
        if (msg.orderType === 'market') {
            if (this.unfilledMarketOrders.has(msg.orderId)) {
                this.emit('LiveOrderbook.executed', msg);
            }
            return;
        }
        this.emit('LiveOrderbook.executed', msg);
        const order: Level3Order = this._book.getOrder(msg.orderId);
        if (!order) {
            this.log('warn', 'Traded order not in my book', msg);
            this.emit('LiveOrderbook.outOfSyncWarning',
    ...
    private handleOrderPlacedConfirmation(msg: MyOrderPlacedMessage) {
        ...
        if (this._book.getOrder(orderId)) {
        ...
        this._book.add(order);
        this.emit('LiveOrderbook.placed', msg);
    }
